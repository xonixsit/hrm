<template>
  ...
  <nav :class="[
  ...
  ]" style="display: block !important; visibility: visible !important;" data-testid="integrated-top-navigation">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between h-20">
        ...
        <div class="hidden lg:flex space-x-1">
          <a v-for="item in mainNavItems" :key="item.route" :href="route(item.route)" :class="[
            ...
            <svg v-if="item.route === 'dashboard'" class="w-4 h-4" fill="none" stroke="currentColor"
              viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2-2z" />
            </svg>
            <svg v-else-if="item.route === 'attendances.index'" class="w-4 h-4" fill="none" stroke="currentColor"
              viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <svg v-else-if="item.route === 'leaves.index'" class="w-4 h-4" fill="none" stroke="currentColor"
              viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <div class="relative">
            <button @click="showMore = !showMore" :class="[
              ...
              <svg :class="['w-4 h-4 transition-transform duration-200', { 'rotate-180': showMore }]" fill="none"
            ...
            <div v-show="showMore" :class="[
              ...
              <div class="p-2">
                <div v-if="moreNavItems.length > 0" class="space-y-1">
                  <!-- Regular Items -->
                  <div class="space-y-1">
                    <a v-for="item in regularMoreItems" :key="item.route" :href="route(item.route)" :class="[
                  ...
                  <div v-if="managementItems.length > 0" :class="[
                    ...
                    <div class="space-y-1 mt-1">
                      <a v-for="item in managementItems" :key="item.route" :href="route(item.route)" :class="[
                            'flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200',
                            isActive(item.route)
                              ? (isDark 
                                  ? 'bg-blue-600/20 text-blue-300 border border-blue-500/30' 
                  ...
                  <div v-if="adminItems.length > 0" :class="[
                    ...
                    <div class="space-y-1 mt-1">
                      <a v-for="item in adminItems" :key="item.route" :href="route(item.route)" :class="[
                            'flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200',
                            isActive(item.route)
                              ? (isDark 
      ...
      <div class="flex items-center space-x-3">
        ...
        <div class="relative">
          <button @click="showUserMenu = !showUserMenu" :class="[
            ...
            <svg :class="['w-4 h-4 transition-transform duration-200', { 'rotate-180': showUserMenu }]" fill="none"
              stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
          </button>
    ...
    <div class="md:hidden px-4 pb-3">
      <button @click="showMobileMenu = !showMobileMenu" :class="[
        ...
        <span>Menu</span>
        <svg :class="['w-5 h-5 transition-transform duration-200', { 'rotate-180': showMobileMenu }]" fill="none"
          stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
        </svg>
      </button>
      ...
      <div v-show="showMobileMenu" class="mt-3 space-y-2">
        <a v-for="item in allNavItems" :key="item.route" :href="route(item.route)" :class="[
