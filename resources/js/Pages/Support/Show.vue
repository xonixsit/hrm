<template>
    <AuthenticatedLayout>
        <PageLayout :title="`Support Request #${supportRequest.id}`" :subtitle="supportRequest.subject"
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6 bg-white">
                            ...
                            <div class="border-b border-gray-200 pb-6 mb-6"
                                :class="getPriorityBorderClass(supportRequest.priority)">
                                <div class="flex items-center justify-between mb-4">
                                    <div class="flex items-center space-x-4">
                                        <div class="flex items-center space-x-2">
                                            <div :class="getPriorityIndicatorClass(supportRequest.priority)"
                                                class="w-4 h-4 rounded-full"></div>
                                            <span :class="getStatusBadgeClass(supportRequest.status)"
                                                class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium">
                                                <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                    <path v-if="supportRequest.status === 'open'" fill-rule="evenodd"
                                                        d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                                                        clip-rule="evenodd"></path>
                                                    <path v-else-if="supportRequest.status === 'in_progress'"
                                                        fill-rule="evenodd"
                                                        d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z"
                                                        clip-rule="evenodd"></path>
                                                    <path v-else-if="supportRequest.status === 'resolved'"
                                                        fill-rule="evenodd"
                                                        d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                                                        clip-rule="evenodd"></path>
                                                </svg>
                                        ...
                                        <span :class="getPriorityBadgeClass(supportRequest.priority)"
                                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium">
                                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                <path v-if="supportRequest.priority === 'urgent'" fill-rule="evenodd"
                                ...
                                <div class="flex items-center space-x-4 text-xs text-gray-500">
                                    ...
                                    <div class="flex items-center space-x-1"
                                        :class="{ 'text-yellow-600': supportRequest.status === 'in_progress' || supportRequest.status === 'resolved' }">
                                        <div class="w-2 h-2 rounded-full"
                                            :class="supportRequest.status === 'in_progress' || supportRequest.status === 'resolved' ? 'bg-yellow-400' : 'bg-gray-300'">
                                        </div>
                                        <span>In Progress</span>
                                    </div>
                                    <div class="w-8 h-px bg-gray-300"></div>
                                    <div class="flex items-center space-x-1"
                                        :class="{ 'text-green-600': supportRequest.status === 'resolved' }">
                                        <div class="w-2 h-2 rounded-full"
                                            :class="supportRequest.status === 'resolved' ? 'bg-green-400' : 'bg-gray-300'">
                                        </div>
                                        <span>Resolved</span>
                                    </div>
                                </div>
                                ...
                                <div class="md:col-span-2 space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <div class="bg-gray-50 rounded-lg p-4">
                                            <p class="text-gray-700 whitespace-pre-wrap">{{ supportRequest.description
                                                }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div v-if="supportRequest.attachments && supportRequest.attachments.length > 0">
                                        ...
                                        <div class="space-y-2">
                                            <div v-for="attachment in supportRequest.attachments" :key="attachment.name"
                                                ...
                                                <span class="text-sm text-gray-700">{{ attachment.name || 'Attachment'
                                                    }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-4">
                                        ...
                                        <dl class="space-y-2">
                                            ...
                                            <div>
                                                <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide">
                                                    Category</dt>
                                                <dd class="text-sm text-gray-900">{{ categories[supportRequest.category]
                                                    }}</dd>
                                            </div>
                                            <div>
                                                <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide">
                                                    Priority</dt>
                                                <dd class="text-sm text-gray-900">{{ priorities[supportRequest.priority]
                                                    }}</dd>
                                            </div>
                                            <div>
                                                <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide">
                                                    Status</dt>
                                                <dd class="text-sm text-gray-900">{{ statuses[supportRequest.status] }}
                                                </dd>
                                            </div>
                                            <div v-if="isAdmin">
                                                <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide">
                                            ...
                                            <div>
                                                ...
                                                <dd class="text-sm text-gray-900">{{
                                                    formatDate(supportRequest.created_at) }}</dd>
                                            </div>
